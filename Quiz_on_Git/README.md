...
yes, all files will be pushed to the remote except .gitignore, because when we do the git add * , it will push the file to the staging area, then the repository wil be udate when we add the git commit command. 
...
``git checkout v2.0``
 ...
 if we do the (git checkout v2.0 command) it will became in the head
